...
export default async function Yandex_DriverRefund(driverProfileId: string, amount: number): Promise<boolean> {
  if (!driverProfileId || amount <= 0) {
    console.error("Invalid parameters for Yandex_DriverRefund");
    return false;
  }
  ...
  try {
    ...
    const requestBody = {
      ...
      description: "Driver balance refund",
    ...
    console.error("Error in Yandex_DriverRefund:", error);
