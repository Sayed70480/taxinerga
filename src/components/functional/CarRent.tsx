...
const CarRent = ({ content }: { content: string }) => {
  const [open, setOpen] = useState(false);

  return (
    <>
      {/* Gift Button */}
      <div onClick={() => setOpen(true)} className="rounded-full border-[2px] border-white bg-yellow-500 bg-opacity-80 p-3">
        <FaTaxi className="animate__animated animate__slow animate__infinite animate__heartBeat m-auto text-2xl text-zinc-900" />
      </div>
